...
class Transaction:
    def __init__(self, env, channel, time, from_node, to_node, amount, verbose):
        self.env = env
...
class Channel:
    ...
    def __init__(self, env, node1, node2, capacity, balances, verbose):
        self.env = env
        self.node1 = node1
        self.node2 = node2
...
def transaction_generator(env, channel, from_node, total_transactions, max_transaction_amount, exp_mean, verbose):
    ...
    for _ in range(total_transactions):
        ...
        t = Transaction(env, channel, env.now, from_node, to_node, amount, verbose)
        env.process(t.run())
...
def sc_DES_no_buffer_fun(initial_balances, total_transactions_0, max_transaction_amount_0, exp_mean_0, total_transactions_1, max_transaction_amount_1, exp_mean_1, max_allowed_delay, verbose):

    env = simpy.Environment()

    channel = Channel(env, 0, 1, sum(initial_balances), initial_balances, verbose)
    env.process(transaction_generator(env, channel, 0, total_transactions_0, max_transaction_amount_0, exp_mean_0, verbose))
    env.process(transaction_generator(env, channel, 1, total_transactions_1, max_transaction_amount_1, exp_mean_1, verbose))
    ...
    success_rate = (channel.successful_transactions[0] + channel.successful_transactions[1]) / (total_transactions_0 + total_transactions_1)
