...
def main():
    ...
    channel = Channel(env, 0, 1, sum(G.initial_balances), G.initial_balances)
    env.process(
        transaction_generator(env, channel, 0, G.total_transactions_0, G.max_transaction_amount_0, G.exp_mean_0))
    env.process(
        transaction_generator(env, channel, 1, G.total_transactions_1, G.max_transaction_amount_1, G.exp_mean_1))
    ...
    success_rate = [channel.successful_transactions[0] / G.total_transactions_0,
                    channel.successful_transactions[1] / G.total_transactions_1]
    if G.verbose:
